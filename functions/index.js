...
exports.allPlayersReady = functions.database
  .ref("/games/{gid}/ready")
  .onWrite((event) => {
    ...
    if (
      playerKeys.filter((key) => playersReady[key] === true).length ===
      playerKeys.length
    ) {
      return round
        .once("value")
        .then((snap) => {
          ...
          return admin
            .database()
            .ref(`games/${event.params.gid}`)
            .update({ round: currentRound + 1, loading: true });
        })
        .catch((e) => {
          console.log(e);
        });
...
exports.roundFinished = functions.database
  .ref("/games/{gid}/roundFinished")
  .onWrite((event) => {
    if (!event.data.exists()) {
    ...
    if (isFinished) {
      ...
      return game
        ...
        .then((snap) => {
          ...
          canMoveOn =
            Object.keys(round.taken).filter(
              (i) => round.taken[i].word.length === 6
            ).length > 0;
          ...
          return admin
            .database()
            .ref("/gameRounds/" + gameRoundKey + "/taken")
            .update(taken);
        })
        .then(() => {
          return admin
...
exports.populateRoundWords = functions.database
  .ref("/games/{gid}/round")
  .onWrite((event) => {
...
exports.addUserToGame = functions.database
  .ref("/games/{gid}/players/{uid}")
  .onWrite((event) => {
    // Exit when the data is deleted.
    if (!event.data.exists()) {
      // TODO: delete '/gamePlayers/gid/uid', set game open to true,
    ...
    return admin
      ...
      .then((snap) => {
        ...
        return admin
          .database()
          .ref(`/games/${event.params.gid}/players`)
          .once("value");
      })
      .then((snap) => {
        players = snap;
      ...
      .then((snap) => {
        ...
        return admin
          .database()
          .ref(`games/${event.params.gid}/currentGamePlayers`)
          .update(updates);
      })
      .then(() => {
        const updates = {};
        ...
        return admin
          .database()
          .ref(`games/${event.params.gid}/ready/`)
          .update(updates);
      })
      .then(() => {
        return admin
          .database()
          .ref(`games/${event.params.gid}`)
          .update({ loading: false, playerCount: playerCount });
      })
      .catch((e) => {
        console.log(e);
      });
...
exports.setUserData = functions.database
  .ref("/users/{uid}")
  .onWrite((event) => {
    // Only edit data when it is first created.
    if (!event.data.exists()) {
...
exports.deleteOldGamePlayers = functions.database
  .ref("/gamePlayers/{pushId}/timestamp")
  .onWrite((event) => {
    if (event.data.previous.exists()) {
      return;
    }
...
exports.deleteOldGames = functions.database
  .ref("/games/{pushId}/timestamp")
  .onWrite((event) => {
    if (event.data.previous.exists()) {
      return;
    }
...
exports.deleteOldGameRound = functions.database
  .ref("/gameRounds/{pushId}/timestamp")
  .onWrite((event) => {
    if (event.data.previous.exists()) {
      return;
    }
...
const assignPointValues = (list) => {
  ...
  const listWithValues = list.reduce(
    (accum, word, i) => {
      let value = 0;
